...
if (count($this->rows) > 0) {
	for ($i=0, $n=count($this->rows); $i<$n; $i++)
	{
		...
		$list .= '</div>';
		$list .= '</a>';
		$list .= '<p style="font-weight: bold;margin-top: 0.8rem;" onclick="App.deleteCollect('.$row->rec_id.',2)">删除</p>';
		$list .= '</div>';
	}
} else {
	$list .= '<p class="fav_nothing" id="shoplist_nothing">您还未收藏任何商品！</p>';
}
?>
	...
	<div class="order-topbar">
		<div class="topbar-tabs">
			<ul id="nav">
				...
				<li class="cur"><a href="#m=user&c=collect">我的收藏</a></li>

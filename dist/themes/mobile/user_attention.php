...
if (count($this->rows) > 0) {
	for ($i=0, $n=count($this->rows); $i<$n; $i++)
	{
		...
		$list .= '</div>';
		$list .= '</a>';
		$list .= '<p style="font-weight: bold;margin-top: 0.8rem;" onclick="App.deleteCollect('.$row->rec_id.',1)">删除</p>';
		$list .= '</div>';
	}
} else {
	$list .= '<p class="fav_nothing" id="shoplist_nothing">您还未关注任何商品！</p>';
}
?>
<div id="content" class="page-content">
	<div class="order-topbar">
		<div class="topbar-tabs">
			<ul id="nav">
				<li class="cur"><a href="#m=user&c=attention">我的关注</a></li>

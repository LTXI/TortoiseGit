 ...
 */
(function(w) {
	var server = "http://www.nineneeds.com/upload/appdownload/update.json", //获取升级描述文件服务器地址
		localDir = "update",
	...
	function checkUpdateData(j) {
		//console.log("11checkUpdateData" + j);
		var curVer = plus.runtime.version,
			inf = j[plus.os.name];
		//console.log("plus.os.name]inf===" + inf);
		if(inf) {
			var srvVer = inf.version;
			//console.log("srvVer===" + srvVer);
			// 判断是否存在忽略版本号
			var vabort = plus.storage.getItem(keyAbort);
			//console.log("vabort===" + vabort);
			if(vabort && srvVer == vabort) {
				// 忽略此版本
				return;
			}
			//console.log("curVer===" + curVer + " srvVer==" + srvVer);
			// 判断是否需要升级
			var isUpgrade = compareVersion(curVer, srvVer);
			//alert(isUpgrade);
			//isUpgrade = true;
			if(isUpgrade) {
				console.log("compareVersion curVer===" + curVer + " srvVer==" + srvVer);
				// 提示用户是否升级
				plus.nativeUI.confirm("当前版本" + curVer + "\n" + inf.note, function(i) {
					if(0 == i.index) {
						if(plus.os.name == "iOS") {
	...
	function getUpdateData() {
		console.log("xhr.readyState== " + server);
					...
					if(xhr.status == 200) {
						...
						dir.getFile(localFile, {
							...
							fentry.createWriter(function(writer) {
								...
								console.log("获取升级数据，保存文件失败！" + xhr.responseText);

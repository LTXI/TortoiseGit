...
.store_logo { text-align:center;width:80%; margin:0 auto; border:1px solid #e4e4e4; font-size:0;}
...
<div id="content" class="page-content">
	<div id="wrapper" class="auth_warp">
		<form name="app-submit" method="post" id="myupload" action="" data-action="m=agent&a=editStoreLogo">
			<div class="head-title">店铺LOGO上传</div>
			<?php 
				if(isset($this->store_logo)){
					echo '<div class="store_logo"><img src="'.JQ_URL.$this->store_logo.'" /></div>';
				}else{
					echo '<div class="store_logo"><img src="'.JQ_URL.'images/no_picture.gif" /></div>';
				}
			?>
			...
			<div class="mod_btns">
				...
				<input type="hidden" id="store_logo" name="store_logo" value="<?php echo $this->store_logo?>">
				<a class="mod_btn bg_2" href="javascript:void(0);" onclick="javascript:App.submit();">提交修改</a>
			</div>
		</form>
	</div>
</div>
...
$(function(){
	...
	$("#fileupload").change(function(){
		...
		$.ajax({
			url: "index.php?m=agent&a=uploadStoreLogo",
			type: "POST",
			data: formdata,
			processData: false,  
			contentType: false,   
